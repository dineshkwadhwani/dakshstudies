...
export default function AdminUsers() {
  ...
  }, [users, search, roleFilter, statusFilter, verificationFilter, me?.role])
  const availableUserCount = me?.role === 'account_manager' ? users.filter(user => user.role === 'student').length : users.length
  const clearFilters = () => { setSearch(''); setRoleFilter('all'); setStatusFilter('all'); setVerificationFilter('all') }
  return <div><PageTitle eyebrow="Platform accounts" title="Users" text={me?.role === 'super_admin' ? 'Create Account Managers and manage platform access.' : 'Search and view all students in the platform.'} />
    ...
    <section className="card p-4 mb-4" aria-label="Filter users">
      <div className={`grid ${me?.role === 'super_admin' ? 'sm:grid-cols-5' : 'sm:grid-cols-4'} gap-3 items-end`}>
        ...
        <button type="button" className="btn-secondary py-3" onClick={clearFilters} disabled={!search && roleFilter === 'all' && statusFilter === 'all' && verificationFilter === 'all'}>Clear</button>
      </div>
