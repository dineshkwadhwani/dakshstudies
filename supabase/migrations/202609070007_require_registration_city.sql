...
declare
  ...
  referrer_id uuid;
  generated_code text;
  supplied_name text := nullif(regexp_replace(trim(new.raw_user_meta_data ->> 'full_name'), '[[:space:]]+', ' ', 'g'), '');
  supplied_phone text := nullif(trim(new.raw_user_meta_data ->> 'phone'), '');
  supplied_city text := nullif(regexp_replace(trim(new.raw_user_meta_data ->> 'city'), '[[:space:]]+', ' ', 'g'), '');
...
begin
  ...
  if not is_admin_provisioned and (
    supplied_city is null or char_length(supplied_city) > 80 or
    supplied_city !~ '^[[:alpha:]][[:alpha:][:space:].''-]{1,79}$'
  ) then
  ...
  insert into public.profiles (
    id, email, email_verified, role, status, full_name, phone, city, onboarding_step,
    ...
    supplied_name, supplied_phone, supplied_city, 'verify_email', generated_code, referrer_id,
