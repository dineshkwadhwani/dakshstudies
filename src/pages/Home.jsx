...
function UpgradePrompt({ expired, packages, session, upgrading, setUpgrading, error, setError }) {
  ...
  async function startUpgrade() {
    ...
    try {
      ...
      setPending(body.payment)
    } catch (upgradeFailure) { setError(upgradeFailure.message || 'Payment could not be started.') } finally { setUpgrading(false) }
  }
  if (pending) return <UpgradeReview payment={pending} session={session} onClose={() => setPending(null)} />
  return <section className={`card p-4 mt-2 ${expired ? 'bg-flame/10 border-flame/50' : 'bg-sun/10 border-ink/20'}`}><div className="font-mono text-[11px] uppercase tracking-widest text-ink/55">{expired ? 'Trial ended' : 'Free trial'}</div><h2 className="font-display font-bold text-lg mt-1">{expired ? 'Your trial package has expired' : 'Explore a paid package'}</h2><p className="text-sm text-ink/65 mt-1">{expired ? 'Upgrade to continue learning, taking quizzes and using mock tests.' : 'Upgrade whenever you are ready for full access to your study plan.'}</p><div className="flex flex-col sm:flex-row gap-2 mt-3"><select className="form-control flex-1" value={selected} onChange={event => setSelected(event.target.value)} disabled={upgrading}>{packages.map(pkg => <option value={pkg.code} key={pkg.code}>{pkg.name} — ₹{Math.round(pkg.price_paise / 100)}</option>)}</select><button type="button" className="btn-primary" disabled={upgrading || !selected} onClick={startUpgrade}>{upgrading ? 'Loading…' : 'View upgrade options →'}</button></div>{!packages.length && <p className="text-sm mt-2">No paid packages are currently available.</p>}{error && <p className="text-sm text-flame font-bold mt-2">{error}</p>}</section>
}

function UpgradeReview({ payment, session, onClose }) {
  const [coupon, setCoupon] = useState(''); const [summary, setSummary] = useState(null); const [busy, setBusy] = useState(false); const [error, setError] = useState('')
  async function calculate() { setBusy(true); setError(''); const response = await fetch('/api/payment-order', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ transactionId: payment.transactionId, couponCode: coupon }) }); const body = await response.json().catch(() => ({})); setBusy(false); if (!response.ok) return setError(body.error || 'Coupon could not be applied.'); setSummary(body) }
  async function pay() { if (!summary) return calculate(); if (!window.Razorpay) await new Promise((resolve, reject) => { const script = document.createElement('script'); script.src = 'https://checkout.razorpay.com/v1/checkout.js'; script.onload = resolve; script.onerror = reject; document.body.appendChild(script) }); const checkout = new window.Razorpay({ key: summary.payment.keyId, amount: summary.payment.amount, currency: summary.payment.currency, order_id: summary.payment.orderId, name: 'Tenth Ki Padhai', description: 'Package upgrade', prefill: { email: session.user.email }, handler: async result => { const verify = await fetch('/api/verify-payment', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ transactionId: summary.payment.transactionId, razorpayOrderId: result.razorpay_order_id, razorpayPaymentId: result.razorpay_payment_id, razorpaySignature: result.razorpay_signature }) }); if (verify.ok) window.location.reload(); else setError('Payment verification did not succeed.') }, modal: { ondismiss: onClose } }); checkout.open() }
  async function cancel() { if (!window.confirm('Your upgrade purchase is not completed. Your current package and expiry date will remain unchanged.')) return; await fetch('/api/payment-fallback', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ transactionId: payment.transactionId, outcome: 'cancelled' }) }).catch(() => null); onClose() }
  return <section className="card p-4 mt-2 bg-sun/10"><div className="font-mono text-[11px] uppercase tracking-widest text-ink/55">Review upgrade</div><h2 className="font-display font-bold text-lg mt-1">Confirm your package upgrade</h2><div className="card p-3 bg-cream mt-3"><div className="text-sm">Amount</div><div className="heading-display text-3xl mt-1">₹{Math.round((summary?.amount || payment.amount) / 100)}</div>{summary?.discount > 0 && <div className="text-sm text-green-700">Discount: −₹{Math.round(summary.discount / 100)}</div>}</div><div className="flex gap-2 mt-3"><input className="form-control flex-1" placeholder="Coupon code" value={coupon} onChange={e => setCoupon(e.target.value.toUpperCase())} disabled={busy || Boolean(summary)} /><button type="button" className="btn-secondary" disabled={busy || !coupon || Boolean(summary)} onClick={calculate}>Apply</button>{summary && <button type="button" className="btn-secondary" onClick={() => setSummary(null)}>Remove</button>}</div>{error && <p className="text-sm text-flame font-bold mt-2">{error}</p>}<div className="grid sm:grid-cols-2 gap-2 mt-3"><button type="button" className="btn-primary" disabled={busy} onClick={pay}>{busy ? 'Checking…' : 'Continue to Razorpay →'}</button><button type="button" className="btn-secondary" disabled={busy} onClick={cancel}>Cancel</button></div></section>
}
