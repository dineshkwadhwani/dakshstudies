...
export async function handleImpersonate(request, response, env = process.env) {
  ...
  if (!UUID.test(userId) || userId === authData.user.id) return json(response, 400, { error: 'A valid target user is required' })
  ...
  const { data: linkData, error: linkError } = await admin.auth.admin.generateLink({
    type: 'magiclink',
    email: target.email,
    options: { redirectTo: `${productionUrl}/dashboard` },
  })
  if (linkError || !linkData?.properties?.hashed_token) return json(response, 400, { error: linkError?.message || 'Unable to create impersonation session' })
  ...
  return json(response, 200, { tokenHash: linkData.properties.hashed_token })
}

export default function handler(request, response) {
  return handleImpersonate(request, response)
}
