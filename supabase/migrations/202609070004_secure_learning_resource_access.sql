...
drop policy if exists "Entitled users can read learning content files" on storage.objects;
create policy "Only SuperAdmin can directly read learning content files"
on storage.objects for select to authenticated
using (bucket_id = 'learning-content' and (select private.is_super_admin()));

drop policy if exists "Active entitled users can view assessment resources" on public.assessment_resources;
create policy "Assessment resources follow release policy"
  ...
  or (
    ...
    and exists (
      ...
      where assessment.id = assessment_id
        ...
        and (
          purpose <> 'answer_key'
          or assessment.answer_key_policy = 'always'
          or exists (
            select 1 from public.assessment_attempts attempt
...
revoke select on public.content_resource_versions from authenticated;
grant select (
  id, resource_id, version, mime_type, size_bytes, checksum_sha256,
  provenance, created_by, reviewed_by, published_at, created_at
) on public.content_resource_versions to authenticated;
