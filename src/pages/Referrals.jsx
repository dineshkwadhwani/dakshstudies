...
export default function Referrals() {
  ...
  const url = `${window.location.origin}/register?ref=${encodeURIComponent(profile?.referral_code || '')}`
  return <div>
    ...
    <section className="card p-5 sm:p-6 bg-sun/15"><div className="text-xs font-mono uppercase text-ink/55">Your referral code</div><div className="heading-display text-3xl mt-1">{profile?.referral_code}</div><div className="flex flex-col sm:flex-row gap-2 mt-4"><input className="form-control mt-0 flex-1 text-sm" value={url} readOnly /><button className="btn-secondary" type="button" onClick={() => navigator.clipboard.writeText(url)}>Copy link</button></div></section>
