...
export default function Progress() {
  ...
  const schedSummary = getScheduleSummary(schedDone, schedule)
  const coverage = getScheduleCoverageBySubject(schedDone, schedule)
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-5">
        <BigStat label="Days fully done" value={`${schedSummary.daysFullyDone}/${schedSummary.totalDays}`} sub="both tasks ticked" color="bg-sun/30" />
        ...
        <BigStat label="Tests" value={testAvgPct !== null ? `${testAvgPct}%` : `${testsDone}`} sub={testAvgPct !== null ? 'avg of entered' : 'attempted'} color="bg-flame/20" />
      </div>
      ...
      <div className="card p-3 mb-5 bg-paper">
        {Object.keys(coverage).length === 0 ? (
          <p className="text-sm text-ink/50">No chapters scheduled yet.</p>
        ) : (
