...
export default function PdfModal({ open, url, title, onClose }) {
  ...
  const useNativeViewer = isIPadOS()
  ...
  return createPortal(<div className="fixed inset-0 z-[100] bg-ink/70 backdrop-blur-sm flex flex-col" role="dialog" aria-modal="true" aria-label={title}>
    ...
    <div ref={contentRef} className={`flex-1 overflow-auto overscroll-contain ${useNativeViewer ? 'bg-paper' : 'p-3 sm:p-5'}`}>
      {useNativeViewer
        ? <iframe src={`${url}#view=FitH`} title={title} className="block w-full h-full border-0 bg-paper" />
    ...
    {!useNativeViewer && pageCount > 0 && <div className="bg-cream border-t-2 border-ink px-3 py-2 flex items-center justify-center gap-3">
