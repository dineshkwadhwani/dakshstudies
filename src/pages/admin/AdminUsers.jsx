...
export default function AdminUsers() {
  ...
  async function load() {
    ...
    setUsers((profiles || []).map(user => {
      ...
      return { ...user, entitlement, package_name: entitlement?.packages?.name || directory?.package_name || null, package_ends_at: entitlement?.ends_at || directory?.package_ends_at || null, package_status: packageState(entitlement) === 'none' ? (directory?.package_ends_at && new Date(directory.package_ends_at).getTime() <= Date.now() ? 'expired' : directory?.package_ends_at ? 'active' : 'none') : packageState(entitlement), account_manager_id: assignment?.account_manager_id || '' }
    }))
  }
  ...
  return <div><PageTitle eyebrow="Platform accounts" title="Users" text={me?.role === 'super_admin' ? 'Create Account Managers and manage platform access.' : 'Search and view all students in the platform.'} />
    ...
    {me?.role === 'super_admin' && <form onSubmit={createManager} autoComplete="off" className="card p-5 sm:p-6 mb-6 bg-violet/10"><div className="mb-5"><div className="font-mono text-xs uppercase tracking-widest text-ink/55">Staff access</div><h2 className="font-display font-extrabold text-xl mt-1">Create Account Manager</h2><p className="text-sm text-ink/60 mt-1">The email is confirmed immediately. Share the temporary password securely.</p></div><div className="grid sm:grid-cols-2 gap-4"><label className="block"><span className="block text-xs font-mono uppercase tracking-wider text-ink/60">Full name</span><input required className="form-control" autoComplete="off" value={form.fullName} onChange={e => setForm({ ...form, fullName: e.target.value })} /></label><label className="block"><span className="block text-xs font-mono uppercase tracking-wider text-ink/60">Email address</span><input required type="email" name="manager-email" autoComplete="new-user-email" className="form-control" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} /></label><label className="block sm:col-span-2"><span className="block text-xs font-mono uppercase tracking-wider text-ink/60">Temporary password</span><input required type="password" name="manager-password" autoComplete="new-password" minLength="12" className="form-control" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} /><span className="text-xs text-ink/55 mt-1 block">Use at least 12 characters with a capital letter, number and symbol.</span></label></div><button disabled={creating} className="btn-primary mt-5">{creating ? 'Creating…' : 'Create Account Manager →'}</button></form>}
